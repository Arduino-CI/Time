 ...
 *
 * No memory is consumed in the sketch if your code does not call any of the
 * string methods You can change the text of the strings, make sure the short
 * strings are each exactly 3 characters the long strings can be any length up
 * to the constant dt_MAX_STRING_LEN defined in TimeLib.h
 *
 */

#include <Arduino.h>

// Arduino.h should properly define PROGMEM, PGM_P, strcpy_P, pgm_read_byte,
// pgm_read_ptr But not all platforms define these as they should.  If you find
// a platform needing these defined, or if any this becomes unnecessary as
// platforms improve, please send a pull req.
#if defined(ESP8266)
#undef PROGMEM
#define PROGMEM
#endif
...
static char buffer[dt_MAX_STRING_LEN + 1]; // must be big enough for longest
...
const PROGMEM char *const PROGMEM monthNames_P[] = {
    monthStr0,  monthStr1,  monthStr2, monthStr3, monthStr4,
...
char *monthShortStr(uint8_t month) {
  for (int i = 0; i < dt_SHORT_STR_LEN; i++)
    buffer[i] =
        pgm_read_byte(&(monthShortNames_P[i + (month * dt_SHORT_STR_LEN)]));
