 ...
 * A Processing example sketch to automatically send the messages is inclided in
 the download
...
void processSyncMessage() {
  ...
  const unsigned long DEFAULT_TIME =
      1357041600; // Jan 1 2013 - paul, perhaps we define in time.h?

  pctime = Serial.parseInt();
  if (pctime >= DEFAULT_TIME) { // check the integer is a valid time (greater
                                // than Jan 1 2013)
    setTime(
        pctime); // Sync Arduino clock to the time received on the serial port
  }
}
