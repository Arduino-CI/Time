...
void setup() {
  ...
  setSyncProvider(
      getDstCorrectedTime); // Use this for local, DST-corrected time
}
...
time_t getDstCorrectedTime(void) {
  ...
  if (t > 0) {
    TimeElements tm;
    breakTime(t, tm);
    t += (utcOffset + dstOffset(tm.Day, tm.Month, tm.Year + 1970, tm.Hour)) *
         SECS_PER_HOUR;
  }
...
byte dstOffset(byte d, byte m, unsigned int y, byte h) {
  ...
  if ((m > 3 && m < 10) || (m == 3 && (d > dstOn || (d == dstOn && h >= 1))) ||
